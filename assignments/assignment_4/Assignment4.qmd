...
# 检查返回的数据
cat("Records returned:", nrow(graffiti_df), "\n")
cat("Columns:", colnames(graffiti_df), "\n")
```
...
if(nrow(graffiti_clean) > 0) {
  ...
  cat("Year range: ", min(graffiti_clean$year, na.rm = TRUE),
      " to ", max(graffiti_clean$year, na.rm = TRUE), "\n")
}

head(graffiti_clean, 5)

# Which columns are retained in the display?
cat("\nColumns in cleaned data:\n")
print(colnames(graffiti_clean))
```
...
for (fold_num in 1:n_folds) {
  ...
  # Fit simple model (without spatial features for now)
  fold_model <- glm(
    n_graffiti ~ 1,
    family = poisson(link = "log"),
    data = train_data
...
```{r prepare_2018_data}
# Redownload 2018 Crime Data - Using Correct API Syntax
crime_url <- "https://data.cityofchicago.org/resource/3i3m-jwuy.json"
response_2018 <- GET(crime_url, query = list(
  "$limit" = 50000
))
...
cat("2018 Crime Data Loaded:\n")
cat("Total records:", nrow(crime_2018_df), "\n")
cat("Columns:", colnames(crime_2018_df), "\n\n")

# Check the data
head(crime_2018_df, 3)

# Filtering 2018 and Burglary Data in R
crime_2018_filtered <- crime_2018_df %>%
  filter(!is.na(longitude) & !is.na(latitude)) %>%
  mutate(
    longitude = as.numeric(longitude),
    latitude = as.numeric(latitude),
...
cat("2018 Burglary Data Filtered:\n")
cat("Records:", nrow(crime_2018_filtered), "\n")
```
...
cat("Crime 2018 cleaned records:", nrow(crime_2018_clean), "\n")
...
cat("2018 Data Converted to SF:\n")
cat("Records:", nrow(crime_2018_utm), "\n")
cat("CRS:", st_crs(crime_2018_utm)$epsg, "\n")
...
pred_2018 <- predict(simple_model, 
