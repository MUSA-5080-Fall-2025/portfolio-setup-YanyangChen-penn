...
**Analysis Goals:**
- Develop a spatial predictive model using k-nearest neighbor and Local Moran's I features
- Compare Poisson and Negative Binomial regression performance
- Validate models using spatial cross-validation (Leave-One-Group-Out)
- Test temporal stability using 2018 crime data
- Evaluate predictive accuracy against kernel density estimation baseline
...
## Downloading Graffiti Data from Chicago Data Portal

The Chicago 311 system records service requests across multiple categories. We focus on Graffiti Removal requests for 2017. The Socrata API allows direct download of filtered datasets.

```{r load_graffiti_2017}
# Chicago 311 API endpoint for graffiti data
api_url <- "https://data.cityofchicago.org/resource/hec5-y4x5.json"

# Query parameters: filter for 2017 data with limit
params <- list(
  "$where" = "date_extract_year(creation_date) = 2017",
  "$limit" = 50000
)
...
cat("Raw 2017 Graffiti Data Summary:\n")
...
graffiti_clean <- graffiti_df %>%
  ...
  select(service_request_id, longitude, latitude, creation_date, year, month, date, address) %>%
...
cat("Date range:", min(graffiti_clean$date), "to", max(graffiti_clean$date), "\n")
...
ggplot() +
  ...
  theme_minimal() +
  labs(
    title = "Spatial Distribution of Graffiti Removal Requests (2017)",
...
ggplot(monthly_stats, aes(x = reorder(month_name, month), y = count)) +
  ...
  theme_minimal() +
  labs(
    title = "Monthly Distribution of Graffiti Requests (2017)",
    x = "Month",
...
We convert the point-based dataset into a regular grid structure for spatial analysis.
...
```{r spatial_weights_matrix}
# Convert to sp object (required by spdep)
graffiti_sp <- as_Spatial(graffiti_grid)

# Create k-nearest neighbors structure (k=5)
...
cat("Type: k-nearest neighbors (k=5)\n")
```

## Computing k-NN Features
...
graffiti_grid <- graffiti_grid %>%
  mutate(neighbor_mean = neighbor_lags)

cat("k-NN Features Computed\n")
cat("Correlation with own count:", 
    round(cor(graffiti_grid$n_graffiti, graffiti_grid$neighbor_mean, 
              use = "complete.obs"), 3), "\n")
```
...
graffiti_grid <- graffiti_grid %>%
  mutate(
    ...
    moran_cluster = case_when(
      ...
      local_i >= 0 & n_graffiti >= median(graffiti_grid$n_graffiti) ~ "High-High",
      local_i >= 0 & n_graffiti < median(graffiti_grid$n_graffiti) ~ "Low-Low",
      local_i < 0 & n_graffiti >= median(graffiti_grid$n_graffiti) ~ "High-Low",
      TRUE ~ "Low-High"
    )
  )

cat("Local Moran's I Analysis:\n")
...
graffiti_grid <- graffiti_grid %>%
  mutate(
    dist_to_hotspot = as.numeric(st_distance(., hotspot_center)) / 1000
  )

cat("Distance to Hotspot Feature:\n")
cat("Hotspot threshold:", hotspot_threshold, "requests\n")
cat("Distance range:", round(min(graffiti_grid$dist_to_hotspot), 2), 
    "to", round(max(graffiti_grid$dist_to_hotspot), 2), "km\n")
```
...
model_data <- graffiti_grid %>%
  st_drop_geometry() %>%
  select(grid_id, n_graffiti, neighbor_mean, local_i, dist_to_hotspot) %>%
  na.omit()
...
if (nb_aic < poisson_aic) {
  cat("→ Negative Binomial model preferred\n")
  selected_model <- nb_model
  model_name <- "Negative Binomial"
} else {
  cat("→ Poisson model preferred\n")
...
cat("\nModel Coefficients (Exponentiated):\n")
print(coef_table)
```

---

# Part 5: Spatial Cross-Validation (2017 Data)
...
cv_data <- graffiti_grid %>%
  st_drop_geometry() %>%
  select(grid_id, n_graffiti, neighbor_mean, local_i, dist_to_hotspot) %>%
  na.omit() %>%
  mutate(fold = spatial_folds[grid_id])
...
for (fold_num in 1:n_folds) {
  ...
  fold_results <- test_data %>%
    ...
    select(fold, grid_id, observed, predicted, error, abs_error, squared_error)
...
cat("SPATIAL CROSS-VALIDATION RESULTS (2017)\n")
cat("=" %*% 50, "\n")
...
```{r load_2018_crimes}
# Download 2018 burglaries (forced entry only)
crime_url <- "https://data.cityofchicago.org/resource/3i3m-jwuy.json"

params_2018 <- list(
  "$where" = "date_extract_year(date) = 2018 AND primary_type = 'BURGLARY' AND description LIKE '%FORCED%'",
  "$limit" = 50000
)

response_2018 <- GET(crime_url, query = params_2018)
...
crime_2018_features <- crime_2018_grid %>%
  st_join(
    graffiti_grid %>%
      select(grid_id, neighbor_mean, local_i, dist_to_hotspot),
    join = st_intersects
  ) %>%
  st_drop_geometry() %>%
  group_by(grid_id) %>%
...
# Comparison
cat("SPATIAL vs TEMPORAL COMPARISON\n")
cat("Spatial CV MAE:", round(mae, 3), "\n")
cat("Temporal MAE:", round(mae_2018, 3), "\n")
cat("Difference:", round(mae_2018 - mae, 3), "\n")
```

---

# Part 7: Baseline Comparison and Final Evaluation

## KDE Baseline

```{r kde_baseline}
# Create point pattern
window <- as.owin(as_Spatial(chicago_utm))
graffiti_ppp <- as.ppp(st_coordinates(graffiti_utm), W = window)
...
kde_errors <- graffiti_grid %>%
  st_drop_geometry() %>%
  select(grid_id, n_graffiti) %>%
  mutate(
    predicted_kde = kde_grid_counts[grid_id],
...
# Compare all methods
comparison <- tibble(
  Method = c(paste("Spatial CV -", model_name), "Temporal (2018)", "KDE Baseline"),
...
cat("1. DATA OVERVIEW\n")
cat("   • 2017 Graffiti requests:", nrow(graffiti_utm), "\n")
cat("   • Grid cells:", nrow(graffiti_grid), "(500m × 500m)\n")
cat("   • Cells with graffiti:", sum(graffiti_grid$n_graffiti > 0), "\n\n")

cat("2. MODEL SELECTED:", model_name, "\n")
cat("   • Spatial CV MAE:", round(mae, 3), "\n")
cat("   • Spatial CV RMSE:", round(rmse, 3), "\n")
cat("   • Improvement over KDE:", round((1 - mae/mae_kde)*100, 1), "%\n\n")

cat("3. TEMPORAL PERFORMANCE\n")
cat("   • 2018 MAE:", round(mae_2018, 3), "\n")
cat("   • Temporal stability:", ifelse(mae_2018 < mae*1.3, "Good", "Degraded"), "\n\n")

cat("4. KEY SPATIAL FEATURES\n")
for (i in 2:nrow(coef_table)) {
  cat("   •", coef_table$term[i], "(IRR =", 
      round(coef_table$estimate[i], 3), ")\n")
}
```
